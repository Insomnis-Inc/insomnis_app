...
class SVFollowers extends StatefulWidget {
  final String id;

  SVFollowers(this.id);
  @override
  State<SVFollowers> createState() => _SVFollowersState();
}

class _SVFollowersState extends State<SVFollowers> {
  List<SVSearchModel> list = [];
  // List<SVSearchModel> tags = [];

  late Future<List<UserApi?>> _users;
  Future<void> _onRefresh() async {
    var res = userFollowers(widget.id);
    setState(() {
      _users = res;
    });
  }
  ...
  @override
  void initState() {
    _users = userFollowers(widget.id);
    super.initState();
    afterBuildCreated(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: svGetScaffoldColor(),
        title: Text('Followers', style: boldTextStyle(size: 20)),
